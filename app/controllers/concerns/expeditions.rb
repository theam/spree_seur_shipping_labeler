module Expeditions
  ...
  def has_seur_shipments?
    @order.shipments && @order.shipments.any? && @order.shipments.first.shipping_method && @order.shipments.first.shipping_method.admin_name.present? && @order.shipments.first.shipping_method.admin_name.downcase.include?('seur')
  end
end